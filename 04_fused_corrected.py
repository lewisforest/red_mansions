...
def deduplicate_candidates(candidates: list) -> list:
    ...
    for c in candidates:
        key = (
            c.get("clue_type"),
            c.get("sentence", "").strip(),
            c.get("matched_text"),
            c.get("stated_age"),
            c.get("bound_type"),
            c.get("raw_target_phrase"),
            c.get("direction"),
            c.get("age_diff"),
            c.get("elapsed_years"),
        )
...
def resolve_segment(
    ...
    # 关键修复：02 输出的所有年龄/时间候选都保留。
    # needs_llm_review=False 的候选不再被删除，也不调用 LLM；
    # needs_llm_review=True 的候选继续走【原来的整段上下文 + 联合候选】LLM 算法。
    direct_age = []
    review_age = []
    for idx, c in enumerate(age_candidates):
        item = dict(c)
        item["_source_index"] = idx
        if c.get("needs_llm_review"):
            review_age.append(item)
        else:
            direct_age.append(item)

    indexed_age_cands = []
    for local_idx, item in enumerate(review_age):
        out = dict(item)
        out["candidate_index"] = local_idx
        indexed_age_cands.append(out)

    # 亲属部分完全保持原算法：原有 candidate 全部送入同一个 LLM prompt。
    ...
    all_prompt_cands = indexed_age_cands + indexed_kinship_cands
    location_highlights = locate_and_build_context(clean_narrative, all_prompt_cands)

    # 只有确实需要复核的年龄才进入 LLM prompt；
    # 但 prompt 的整体结构、上下文、亲属候选和 reasoning 要求保持原样。
    prompt = f"""
【前文背景摘要】：
...
"""
    ...
    # 先准备直接保留的年龄结果。这里只复制 02 的机器候选信息，
    # 不擅自改变原算法对其“高置信”的语义判断。
    resolved_age_map = {}
    for item in direct_age:
        source_idx = item["_source_index"]
        resolved_age_map[source_idx] = {
            "candidate_index": source_idx,
            "reasoning": "[02预扫描直通] 该候选 needs_llm_review=false，04 不重新解释，仅保留 02 的明确候选结果，供人工复核。",
            "is_valid": True,
            "type": "idiom_age" if item.get("clue_type") == "idiom_age" else (
                "bounded" if item.get("bound_type") not in (None, "exact") else "absolute"
            ),
            "subject": (item.get("candidate_characters") or [None])[0],
            "value": item.get("stated_age"),
            "event_context": None,
            "temporal_anchor": None,
            "error_type": None,
            "bound_type": item.get("bound_type", "exact"),
            "resolution_source": "prescan_passthrough",
        }

    # 如果没有任何需要 LLM 的年龄，但存在亲属候选，仍按原算法调用 LLM。
    if not indexed_age_cands and not indexed_kinship_cands:
        return {
            "resolved_ages": [resolved_age_map[i] for i in sorted(resolved_age_map)],
            "resolved_kinships": [],
            "supplementary_findings": [],
            "_error": False,
        }
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if isinstance(res, dict) and any(k in res for k in ["age_verifications", "kinship_verifications", "resolved_ages", "resolved_kinships"]):
                ...
                resolved_review_ages = enforce_candidate_coverage(
                    ages_raw, len(indexed_age_cands), is_age=True
                )
                resolved_kinships = enforce_candidate_coverage(
                    kinships_raw, len(indexed_kinship_cands), is_age=False
                )

                # LLM 的 candidate_index 是 review_age 内部索引，重新映射回原 age_candidates。
                for local_idx, item in enumerate(resolved_review_ages):
                    if local_idx < len(indexed_age_cands):
                        source_idx = indexed_age_cands[local_idx]["_source_index"]
                        item["candidate_index"] = source_idx
                        resolved_age_map[source_idx] = item

                resolved_ages = [
                    resolved_age_map[i] for i in sorted(resolved_age_map)
                ]

                # 兼容原来的输出结构；新增 source 信息但不改变既有字段。
                return {
    ...
    if os.path.exists(INPUT_AGE):
        with open(INPUT_AGE, "r", encoding="utf-8") as f:
            for item in json.load(f):
                ...
                # 只修复“丢失02年龄候选”的问题：所有 age_candidates 都进入04。
                # relation_candidates 保持原有行为，继续进入同一时间线校勘集合。
                cands = list(item.get("age_candidates", []))
